...
from schemas import RequestUsers, UserResponse
import crud
...
async def get_all_users(db: Session = Depends(get_db)):
    ...
    return UserResponse(users=_users).dict(exclude_none=True)
...
async def update_users(request: RequestUsers, db: Session = Depends(get_db)):
    ...
    return UserResponse(users=_users)
...
async def delete_users(id: int, db: Session = Depends(get_db)):
    ...
    return UserResponse().dict(exclude_none=True)
